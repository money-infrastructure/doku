...
(Work in progress - Last update: 2018-04-01)
...
The identity blockchain tree together with the user data blockchain trees can be considered as one large tree in which each right has a globally unique address. The first part of the path uniquely identifies the right holder and the second part leads to the right itself.
...
. One of the unknown backup server must be hacked to steal the backup.
...
Already today it is objectively impossible even for lawyers to completely understand a single area of law. And even if everyone could recite all the laws and regulations by heart, there are so many different opinions that the outcome would not be much better.

In order to smash this Gordian knot, legal standards and autonomous law are introduced.
...
In the implementation of self-enforcing rights RSB contracts play a crucial role. Through RSB contracts, the system is not only aware of the agreed claims and their due dates, but also the performances provided. If a borrower, e.g. does not pay its installments, then the creditor may cause his contract to send a default summons to the Identity Server. On the next contact between the debtor and the identity server, the decision is transmitted to the affected debtor's contract. If the claim is justified, then a prioritized compulsory payment is inserted in the outgoing payments of the debtor.

The consequence is that the borrower can not make other payments until he has cleared the compulsory payment.
...
Trust in a mass society is a difficult task. The Compliance Index is a weighted measure of sanctity of contracts.

A Compliance Index of 100% means that the person has fulfilled his contractual obligations in full and in good time.
