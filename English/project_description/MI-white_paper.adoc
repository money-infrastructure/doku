...
(Work in progress - Last update: 2018-04-03)
...
Today, there is an oversupply of financial services and financial instruments worldwide. Many of them are incomprehensible and intransparent, even to experts. As early as 2006, the global supply of financial market products was six times greater than the gross domestic product of the entire world.

Despite this glut, the financial sector has left its serving role for the real economy in many areas, with the sole aim of making more money out of money.
...
These ‘_none performance-related incomes_’ are considered completely normal and perfectly alright by the recipients. It is ignored that in the macroeconomic context, these incomes predominantly go to the disadvantage of the middle and lower classes.

And also the current cryptocurrencies, such as bitcoin do not represent a real alternative, because by decoupling money production from any institutional or governmental control, the failures in the financial sector will only be driven to new and so far unknown levels.
...
The identity blockchain tree together with the user data blockchain trees can be considered as one large tree spanning across the internet in which each right has a globally unique address. The first part of the path uniquely identifies the owner of a right and the second part leads to the right itself.
...
. If digital bearer instruments are transferred as in payments, points 1. - 8. would have to be made for each transfer along the entire chain. For payments, there would also arise a difference between the total amount of the cash accounts in the system and the external escrow account. At the latest here, the manipulation would be noticed and could be traced back to the origin.
. All break-ins and manipulations would have to be done in a very tight time frame, because the normal use of the system could permanently change the blockchain involved. While an attacker manipulated a particular blockchain backup, this blockchain could be updated and the backups moved to completely different backup servers.
...
The more voluminous and complex a legal system becomes, the less it is generally understood. Too many laws are the nation of law's death.
...
Self-enforcing rights is a concept in which legal claims can be enforced without courts and bailiffs.

Ordinary jurisdiction is not meant to be replaced, but relieved from cases that can be decided on the basis of indisputable and sufficient facts. If one of the contracting parties does not agree with the measures carried out, ordinary legal remedies remain open. However, the chances of getting a different verdict are unlikely, especially since the contracting parties agreed with the procedure and also know exactly what to expect.

In the implementation of self-enforcing rights RSB contracts play a crucial role. Through RSB contracts, the system is not only aware of the agreed claims and their due dates, but also the performances provided. For example, if a borrower does not pay his installments, the lender may initiate his contract to send a dunning notice to the Identity Server. On the next contact between the debtor and the identity server, the notice is transmitted to the affected debtor's contract. If the claim is justified, then a prioritized compulsory payment is inserted in the outgoing payments of the debtor.

The consequence is that the borrower can not make any other payments until he has cleared the compulsory payment.
...
Trust in a mass society is a difficult task. The _Compliance Index_ is a weighted measure of sanctity of contracts.
...
An index over 100 points should be called _prime compliance_ and not only a visible sign to others, but also be associated with privileges in the system.
...
Prime compliance should also be a requirement when a person offers asset management services.
