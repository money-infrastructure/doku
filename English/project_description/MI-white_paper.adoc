...
Even if individual barriers can be taken, it is very unlikely that all barriers will be taken as required. On the one hand, the security concept is based on cryptographic methods, and on the other hand, the effort for manipulation is set to the greatest possible disproportion to the potential yield. In addition, every user can choose to protect their data according to their own needs and options.

The attacker would also have to pass unnoticed at the permanent internal test runs.
